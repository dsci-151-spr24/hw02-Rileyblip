...
***Psychology has the highest percentage of women.***
...
***The mean is not always helpful when it comes to high or low data.***
...
***I used the binwidth of \$5000 because the median is easier to read than the \$1000 binwidth, which labels the medians in scientific notation.***
...
***Out of these summary statistics, the useful one for describing distribution is the median, which is 36000.***
...
college_recent_grads %>%
  ...
  summarise(college_recent_grads = max(median)) %>%
...
***The least popular major category is interdisciplinary.***
...
***All of these STEM majors have median salaries less than the median for all majors' median earnings.***
...
***There are many women without STEM majors that have little median income, while there are few women with STEM majors that have a high median income.***

```{r}
college_recent_grads %>%
  ggplot(data = college_recent_grads, mapping = aes(x = median, y = women))+
...
***What is the relationship between a major and its category?***

***There are majors that come from the same major category.***
```{r eval=TRUE}
college_recent_grads %>%
  arrange(major) %>%
  select(rank, major, major_category)
```
